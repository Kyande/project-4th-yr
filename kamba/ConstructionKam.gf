...
--   open SyntaxKam, SymbolicKam, ParadigmsKam, (L = LexiconKam), (E = ExtendKam), (G = GrammarKam), (R = ResKam), (Sy = SyntaxKam), (S = StructuralKam), Prelude in {
...
--   is_right_VP = mkVP (ParadigmsKam.mkA "right") ;
--   is_wrong_VP = mkVP (ParadigmsKam.mkA "wrong") ;
...
--   lin
--   weekdayPunctualAdv w = SyntaxKam.mkAdv on_Prep (mkNP w) ;         -- on Sunday
--   weekdayHabitualAdv w = SyntaxKam.mkAdv on_Prep (mkNP aPl_Det w) ; -- on Sundays
--   weekdayNextAdv w = SyntaxKam.mkAdv (mkPrep "next") (mkNP w) ;     -- next Sunday
--   weekdayLastAdv w = SyntaxKam.mkAdv (mkPrep "last") (mkNP w) ;     -- last Sunday

--   monthAdv m = SyntaxKam.mkAdv in_Prep (mkNP m) ;
--   yearAdv y = SyntaxKam.mkAdv in_Prep y ;
--   dayMonthAdv d m = ParadigmsKam.mkAdv ("on" ++ d.s ! R.NPAcc ++ m.s ! R.Sg ! R.Nom) ; -- on 17 May
--   monthYearAdv m y = SyntaxKam.mkAdv in_Prep (mkNP (mkCN m y)) ; -- in May 2012
--   dayMonthYearAdv d m y = ParadigmsKam.mkAdv ("on" ++ d.s ! R.NPAcc ++ m.s ! R.Sg ! R.Nom ++ y.s ! R.NPAcc) ; -- on 17 May 2013
...
-- lin InLanguage l = SyntaxKam.mkAdv in_Prep (mkNP l) ;
...
-- lin kamba_Language = mkLanguage "Kamba" ;
